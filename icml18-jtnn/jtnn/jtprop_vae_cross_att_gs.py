...
class JTPropVAE(nn.Module):
    def __init__(self, vocab,  hidden_size, latent_size, depth, model_pooling='average', out_size=1, hidden_multiplier=1, task_type='reg', dropout=0.25, res_block = 2):
        ...
        self.task_type = task_type  # 'reg' or 'clf'
        self.embed_dim = hidden_size  # Original hidden_size for cross-attention
        ...
        if task_type == 'reg':
            ...
            self.prop_loss = nn.CrossEntropyLoss()  # Classification task loss
        else:
            raise ValueError("Invalid task type! Choose 'reg' or 'clf'")
    ...
    def encode_for_prop(self, mol_batch):
    ...
    def forward(self, mol_batch, beta, wandb_run=None, total_step_count=0):
        ...
        prop_output = self.propNN(all_vec).squeeze()

        if self.task_type == 'reg':
            # Regression task: Use MSE loss
            prop_loss = self.prop_loss(prop_output, prop_label)
        elif self.task_type == 'clf':
    ...
    def cross_att_norm(self, tnode_vecs, trans_logits):
        ...
        tran_value, tran_attn_weights = self.cross_attn(trans_logits, tnode_vecs) # trans_logits: torch.Size([8, 65, 420]), tnode_vecs: torch.Size([8, 20, 420])
...
class ResidualBlock(nn.Module):
    def __init__(self, hidden_size, dropout_rate=0.25):
        ...
        self.drop_pre_ip1 = nn.Dropout(p=dropout_rate)
